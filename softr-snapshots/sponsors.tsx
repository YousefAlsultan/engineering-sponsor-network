...
export default function Block(){
  ...
  const headline = useTextSetting({name:"headline",label:"Headline",initialValue:"Find sponsors. Build bigger."});
  ...
  const engineerImage = useImageSetting({name:"engineerImage",label:"Engineer hero image",initialValue:{src:"https://images.unsplash.com/photo-1581094794329-c8112a89af12?auto=format&fit=crop&w=1400&q=86",alt:"Engineer working in a technical environment"}});
  ...
  const teamImage = useImageSetting({name:"teamImage",label:"Engineering team image",initialValue:{src:"https://images.unsplash.com/photo-1551836022-d5d88e9218df?auto=format&fit=crop&w=1200&q=86",alt:"Team collaborating on a project"}});
  ...
  return <div className="min-h-screen bg-[#f5f5f7] text-[#111] selection:bg-blue-200">
    ...
    <nav className="sticky top-0 z-40 border-b border-black/[.055] bg-white/75 backdrop-blur-2xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-7 lg:px-8">
        ...
        <div className="hidden items-center gap-7 text-xs font-medium text-black/58 md:flex">
          <button onClick={scrollDirectory} className="transition hover:text-black">Find Sponsors</button><a href="#how" className="transition hover:text-black">How It Works</a>
    ...
    <header className="relative overflow-hidden bg-[#05070b] text-white">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 py-20 sm:px-7 sm:py-16 lg:grid-cols-[1.03fr_.97fr] lg:px-8 lg:py-16">
        ...
        <div className="relative mx-auto w-full max-w-xl lg:max-w-none">
          ...
          <div className="absolute -bottom-5 -left-3 rounded-2xl border border-white/12 bg-white/95 px-4 py-3 text-black shadow-2xl"><p className="text-[10px] font-semibold uppercase tracking-[.12em] text-black/35">Directory</p><p className="mt-0.5 text-lg font-semibold">{sponsors.length} sponsor programs</p></div>
        </div>
      </div>
    </header>
    ...
    <main id="directory" className="scroll-mt-16 bg-[#f5f5f7]">
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-7 lg:px-8">
        ...
        <div className="mb-8 rounded-[32px] border border-black/[.055] bg-white/82 p-4 shadow-[0_18px_60px_rgba(0,0,0,.07)] backdrop-blur-2xl sm:p-5">
          <label className="flex min-h-14 items-center rounded-[20px] bg-[#f5f5f7] px-4 transition focus-within:bg-white focus-within:ring-4 focus-within:ring-blue-500/10"><Search className="mr-3 h-4 w-4 text-black/32"/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search companies, sponsorships, manufacturing, software..." className="w-full bg-transparent text-sm outline-none placeholder:text-black/30"/>{query&&<button onClick={()=>setQuery("")} className="grid h-8 w-8 place-items-center rounded-full bg-white text-black/45 shadow-sm transition hover:bg-black hover:text-white"><X className="h-4 w-4"/></button>}</label>
          <div className="mt-4 space-y-3">
            <FilterStrip labelText="Team" value={team} allLabel="All teams" options={teamOptions} onChange={setTeam}/>
            <FilterStrip labelText="Support" value={type} allLabel="All sponsorships" options={typeOptions} onChange={setType}/>
            <FilterStrip labelText="Industry" value={industry} allLabel="All industries" options={industryOptions} onChange={setIndustry}/>
            <FilterStrip labelText="Region" value={region} allLabel="U.S. eligible" options={["All regions","Eligibility unknown"]} onChange={setRegion}/>
            <FilterStrip labelText="Status" value={applicationStatus} allLabel="All statuses" options={["Open","Ongoing","Seasonal","Closed","Unknown"]} onChange={setApplicationStatus}/>
            <FilterStrip labelText="Dates" value={dateView} allLabel="Current" options={["Expired","All"]} onChange={setDateView}/>
          </div>
          {(query||team!=="All teams"||type!=="All sponsorships"||industry!=="All industries"||dateView!=="Current"||region!=="U.S. eligible"||applicationStatus!=="All statuses")&&<div className="mt-4 flex justify-end"><button onClick={clearFilters} className="inline-flex items-center gap-1.5 rounded-full bg-black px-4 py-2 text-[11px] font-semibold text-white transition hover:bg-[#0071e3]"><X className="h-3.5 w-3.5"/> Clear filters</button></div>}
        </div>
        ...
        <div className="mb-5 flex items-center justify-between"><span className="text-sm font-medium text-black/50">{filtered.length} {filtered.length===1?"sponsor":"sponsors"}</span><span className="hidden text-[11px] text-black/32 sm:inline">Hover a card to preview interaction</span></div>
...
function ApplicationModal({sponsor,onClose}:{sponsor:any;onClose:()=>void}){
  const f=sponsor.fields,company=label(f.company)||"Sponsor",program=label(f.program),contact=label(f.contactEmail);
  const create=useRecordCreate({from:ds.applications,fields:applicationFields});
  const [step,setStep]=useState(1); const [done,setDone]=useState(false); const [submissionError,setSubmissionError]=useState("");
  ...
  const submit=async()=>{
    ...
    setSubmissionError("");
    try{
      const payload:any={application:`${form.teamName} → ${company}`,sponsorCompany:[sponsor.id],university:form.university,teamName:form.teamName,teamType:form.teamType,applicantName:form.applicantName,applicantEmail:form.applicantEmail,sponsorshipTypes:form.sponsorshipTypes[0],requestedSupport:form.requestedSupport,whyFit:form.whyFit,projectDescription:form.projectDescription,sponsorOffer:form.sponsorOffer,applicationStatus:"Submitted",submittedDate:new Date().toISOString().slice(0,10),deliveryStatus:"Not Sent"};

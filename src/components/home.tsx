...
export default function Block() {
  ...
  return (
    <div className="w-full min-h-screen bg-[#f3f4f6] text-[#151515] selection:bg-[#8ec5ff]/60">
      ...
      <nav className="sticky top-0 z-40 border-b border-black/[0.06] bg-white/82 backdrop-blur-2xl supports-[backdrop-filter]:bg-white/72">
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-5 sm:px-7 lg:px-8">
          ...
          <div className="hidden items-center gap-8 text-[12px] font-medium text-black/65 md:flex">
            ...
            <button onClick={scrollToSponsors} className="rounded-full bg-black px-4 py-2 text-white transition hover:bg-black/80">Explore {sponsors.length} sponsors</button>
          </div>
          <button onClick={() => setMenuOpen((v) => !v)} aria-label="Toggle navigation" aria-expanded={menuOpen} className="grid h-9 w-9 place-items-center rounded-full bg-black text-white md:hidden"><Menu className="h-4 w-4" /></button>
        </div>
        {menuOpen && <div className="border-t border-black/[0.06] bg-white px-5 py-4 md:hidden"><div className="flex flex-col gap-1 text-sm font-medium"><a href="#team-types" onClick={() => setMenuOpen(false)} className="rounded-xl px-3 py-3 hover:bg-black/5">Teams</a><a href="#how-it-works" onClick={() => setMenuOpen(false)} className="rounded-xl px-3 py-3 hover:bg-black/5">How It Works</a></div></div>}
      </nav>
        ...
        <div className="relative mx-auto max-w-7xl px-5 pb-20 pt-16 sm:px-7 sm:pb-28 sm:pt-24 lg:px-8 lg:pb-32 lg:pt-28">
          <div className="grid items-center gap-14 lg:grid-cols-[1.08fr_.92fr]">
            <div>
              ...
              <h1 className="max-w-4xl text-[50px] font-semibold leading-[.96] tracking-[-0.058em] sm:text-[68px] lg:text-[84px]">Connect with industry.<br /><span className="bg-gradient-to-r from-[#8bc7ff] via-white to-[#b5a5ff] bg-clip-text text-transparent">Build bigger.</span></h1>
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <button onClick={scrollToSponsors} className="group inline-flex items-center justify-center gap-1.5 rounded-full bg-white px-6 py-3.5 text-[15px] font-semibold text-black shadow-[0_15px_45px_rgba(255,255,255,.12)] transition hover:bg-white/90 active:scale-[.99]">Explore Directory <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></button>
        ...
        <section className="mx-auto max-w-7xl px-5 py-16 sm:px-7 sm:py-20 lg:px-8 lg:py-24">
          ...
          <div className="sticky top-[68px] z-30 mb-8 rounded-[26px] border border-black/[0.06] bg-white/88 p-3 shadow-[0_12px_40px_rgba(0,0,0,.065)] backdrop-blur-2xl sm:p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
              <label className="relative flex min-h-12 flex-1 items-center rounded-2xl bg-[#f3f4f6] px-4 ring-1 ring-black/[0.025] transition focus-within:bg-white focus-within:ring-[#0066cc]/30"><Search className="mr-3 h-4 w-4 text-black/40" /><input aria-label="Search sponsors" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search companies, sponsorships, or support..." className="w-full bg-transparent text-[14px] outline-none placeholder:text-black/35" />{query && <button onClick={() => setQuery("")} aria-label="Clear search" className="rounded-full p-1 text-black/35 hover:bg-black/5 hover:text-black"><X className="h-4 w-4" /></button>}</label>
      ...
      <StudentCompanyStory onExplore={scrollToSponsors} />

      <section id="how-it-works" className="bg-white">
        <div className="mx-auto max-w-7xl px-5 py-20 sm:px-7 lg:px-8 lg:py-28">
          <div className="grid gap-12 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
            <div><p className="text-sm font-semibold text-[#0066cc]">How it works</p><h2 className="mt-3 text-4xl font-semibold leading-[1.02] tracking-[-0.05em] sm:text-5xl">Three steps from need to request.</h2><p className="mt-5 max-w-md text-[15px] leading-7 text-black/50">The preview stays intentionally simple: discover a relevant sponsor, understand what they offer, then prepare one clear request.</p></div>
...
function SponsorDialog({ record, onApply }: { record: any; onApply: () => void }) {
  ...
  return <DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto rounded-[30px] border-0 bg-white p-0 shadow-2xl sm:rounded-[34px]">
    ...
    <div className="space-y-7 px-6 pb-8 pt-7 sm:px-9 sm:pb-10">
      ...
      <div className="grid gap-2 sm:grid-cols-[1fr_auto]"><button disabled={!contact && !official} onClick={contact ? onApply : () => window.open(official,"_blank","noopener,noreferrer")} className="group flex w-full items-center justify-center gap-2 rounded-full bg-[#0066cc] px-6 py-3.5 text-[14px] font-semibold text-white shadow-[0_10px_30px_rgba(0,102,204,.22)] transition hover:bg-[#0071e3] active:scale-[.99]">{contact ? "Apply for Sponsorship" : official ? "View Official Program" : "Application route not available yet"} <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></button>{official&&<button onClick={()=>window.open(official,"_blank","noopener,noreferrer")} className="flex items-center justify-center gap-2 rounded-full bg-[#f3f4f6] px-5 py-3.5 text-xs font-semibold text-black/60 hover:bg-black/10">Official source <ExternalLink className="h-3.5 w-3.5" /></button>}</div>
...
function ApplicationDialog({ sponsor, onClose }: { sponsor: any; onClose: () => void }) {
  const f=sponsor.fields; const company=toLabel(f.company)||"Sponsor"; const program=toLabel(f.program); const contactEmail=!!f.hasContactRoute; const official=safeUrl(f.officialUrl)||safeUrl(f.alternativeUrl)||safeUrl(f.website); const sponsorTypes=toLabels(f.types);
